...
class Feed extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}`, {
      ...
      body: JSON.stringify(graphQlgetStatusQuery)
    })
      .then(res => {
        return res.json();
      })
  ...
  statusUpdateHandler = event => {
    ...
    const graphQlUserStatusUpdateQuery = {
      query: `
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}`, {
      ...
      body: JSON.stringify(graphQlUserStatusUpdateQuery)
    })
      .then(res => {
        return res.json();
      })
  ...
  finishEditHandler = postData => {
    ...
    fetch(`${process.env.REACT_APP_BASE_IMAGE_URL}/post-image`,{
      method: 'PUT',
      headers: {
        Authorization: 'Bearer ' + this.props.token,
      },
    ...
    .then(fileResData=>{
      ...
      if(this.state.editPost){
        graphqlQueryPost = {
          query:`
            mutation {
              updatePost(postId: "${this.state.editPost._id}", postInput: {title: "${postData.title}", imageUrl: "${imageUrl}", content: "${postData.content}"}) 
              {
                _id
                title
                content
  ...
  deletePostHandler = postId => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}`, {
      ...
      body: JSON.stringify(graphQlDeletePostQuery)
    })
      .then(res => {
        return res.json();
      })
      .then(resData => {
        if (resData.errors) {
          throw new Error('Deletion of Post failed.');
        }
        console.log(resData);
        this.loadPosts();
        // this.setState(prevState => {
